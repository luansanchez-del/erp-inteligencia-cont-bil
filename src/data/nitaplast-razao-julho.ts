...
function mapearEntrada(gerencial: string, cc: string): MapeamentoEntrada | null | "duplicado-fiscal" {
  ...
  if (gerencial === "11.01.002") {
    ...
    if (cc === "102") return { debitoCodigo: "3244", creditoCodigo: "1496", status: "revisar", observacao: "Mapeamento de junho reaproveitado; natureza dos materiais e fornecedor analítico ainda precisam de conferência." };
    return null;
  }
  if (gerencial === "11.01.003") return { debitoCodigo: "3093", creditoCodigo: "1496", status: "revisar", observacao: "Matéria-prima/insumos conforme junho; contrapartida mantida em Fornecedores Diversos até abertura analítica do fornecedor/importação." };
  if (gerencial === "11.02.001") return { debitoCodigo: "25937", creditoCodigo: "1496", status: "validado" };
...
const lancamentosEntradasCcJulho: LancamentoIntegrado[] = entradasCcAgregadasJulho.flatMap((linha, index) => {
  const mapa = mapearEntrada(linha.gerencial, linha.cc);
  ...
  return [{
    ...
    historico: `${linha.descricaoGerencial} - CC ${linha.cc} ${linha.centroCusto}`,
    documento: `${linha.gerencial} / ${linha.documentos} doc(s)`,
    cc: linha.cc,
    centroCusto: linha.centroCusto,
    valor: linha.valor,
    status: mapa.status,
    observacao: mapa.observacao ?? "Mapeamento reaproveitado do mesmo gerencial/CC validado em junho; valor conforme relatório Softdib de julho.",
...
export const valorEntradasMapeadasJulho = arred(lancamentosEntradasCcJulho.reduce((t, l) => t + l.valor, 0));
