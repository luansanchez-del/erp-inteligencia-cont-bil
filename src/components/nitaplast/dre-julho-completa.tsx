...
import { calcularApuracaoIrpjCsllJulho, contaIrrfAplicacoesFinanceirasNitaplast } from "@/data/nitaplast-irpj-csll-julho";
...
export function DreJulhoCompleta() {
  ...
  return <div className="grid gap-5">
    ...
    <Card className="border-amber-500/40 bg-amber-50/40"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">Apuração IRPJ/CSLL — Lucro Real mensal (antecipação de DARF) — 07/2026</CardTitle><CardDescription>Base = lucro contábil do próprio mês, ajustado pelas adições/exclusões do LALUR. Antecipação mensal, sujeita a ajuste anual pelo LALUR.</CardDescription></div><Badge variant="outline" className="border-amber-600 text-amber-800">SUGERIDO PELO SISTEMA — validar com o contador antes de gerar DARF</Badge></div></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><tbody>
      <tr className="border-b font-semibold"><td className="py-2">Lucro Contábil do mês (Resultado da DRE de julho)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.lucroContabilDoMes)}</td></tr>
      <tr className="border-b font-semibold"><td className="py-2">Base IRPJ (lucro do mês ± adições/exclusões do LALUR)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.baseIrpj)}</td></tr>
      ...
      <tr className="border-b font-semibold"><td className="py-2">Base CSLL (lucro do mês ± adições/exclusões do LALUR)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.baseCsll)}</td></tr>
    ...
    </tbody></table></div><p className="mt-3 text-xs text-muted-foreground">Nenhuma adição/exclusão do LALUR foi informada para julho até o momento (multas indedutíveis, doações, compensação de prejuízo fiscal etc.) — a base fica igual ao lucro contábil do mês até o contador informar valores específicos.</p></CardContent></Card>
