...
export function DreJulhoCompleta() {
  ...
  const linhas: Linha[] = [
    ...
    { id: "alienacao", descricao: "Resultado na Alienação de Imobilizado — Matriz", nivel: 0, valor: dre.resultadoAlienacaoImobilizado, criterio: "As três vendas fiscais são mostradas. Mini Cooper e Corolla já estão contabilizados no Razão e no ganho; o transformador está identificado fiscalmente, mas continua pendente de residual." },
    { id: "alien-vendas-fiscais", descricao: "(+) Vendas de Ativo Imobilizado identificadas fiscalmente", nivel: 1, valor: dre.vendasAtivoImobilizadoFiscais, criterio: "Total fiscal de julho: Mini Cooper R$ 119.900,00 + Corolla R$ 127.000,00 + Transformador seco 1000KVA NF 93639 R$ 60.000,00 = R$ 306.900,00. Os R$ 60.000,00 do transformador não entram no ganho enquanto o residual não for confirmado.", status: "pendente" },
    { id: "alien-rec", descricao: "Vendas reconhecidas no Razão — Mini + Corolla", nivel: 2, valor: dre.receitaAlienacaoImobilizado, criterio: "Conta 4736. NF 93495 R$ 119.900,00 + NF 93569 R$ 127.000,00 = R$ 246.900,00.", composicao: base.alienacaoReceita },
    { id: "alien-transf", descricao: "Venda fiscal — Transformador seco 1000KVA (NF 93639)", nivel: 2, valor: dre.vendaAtivoImobilizadoPendente, criterio: "Venda fiscal comprovada de R$ 60.000,00. Valor residual/custo contábil ainda pendente; por isso a venda é exibida para conferência, mas não é somada ao ganho nem ao resultado contábil enquanto a baixa não estiver completa.", status: "pendente" },
    { id: "alien-custo", descricao: "(-) Custo dos Ativos Imobilizados Vendidos — Mini + Corolla", nivel: 1, valor: dre.custoAlienacaoImobilizado, criterio: "Conta 4760. Mini residual R$ 52.500,00 + Corolla residual R$ 93.139,29.", composicao: base.alienacaoCusto },
    { id: "alien-res", descricao: "(=) Ganho reconhecido na Alienação — Mini + Corolla", nivel: 1, valor: dre.resultadoAlienacaoImobilizado, criterio: "R$ 67.400,00 Mini + R$ 33.860,71 Corolla = R$ 101.260,71. O transformador permanece fora deste ganho até confirmação do residual." },
    { id: "resultado", descricao: "(=) RESULTADO CONTÁBIL 07/2026", nivel: 0, valor: dre.resultado, criterio: "Resultado do Razão incluindo Mini + Corolla. A venda fiscal do transformador está visível, mas seu ganho/perda permanece pendente até a baixa contábil completa." },
  ];
  ...
  function exportarDreExcel() {
    ...
    exportarExcel({ arquivo: "Nitaplast_DRE_Report_072026.xlsx", aba: "DRE 07-2026", titulo: "NITAPLAST IND E COM DE PLÁSTICOS INDUSTRIAIS LTDA — DEMONSTRAÇÃO DO RESULTADO DO EXERCÍCIO", subtitulo: "Período 01/07/2026 a 31/07/2026 · Razão → Balancete → DRE · Matriz e Filial SP segregadas · vendas fiscais de imobilizado R$ 306.900,00 · transformador R$ 60 mil com residual pendente", colunas: [{ cabecalho: "Descrição", largura: 86 }, { cabecalho: "Valor", largura: 18, tipo: "numero" }, { cabecalho: "% Receita", largura: 14, tipo: "percentual" }], linhas: linhasExcel });
  }
