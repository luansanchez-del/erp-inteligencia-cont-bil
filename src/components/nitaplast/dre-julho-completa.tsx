...
import { calcularApuracaoIrpjCsllJulho, contaIrrfAplicacoesFinanceirasNitaplast, percentualPresuncaoCsllNitaplast, percentualPresuncaoIrpjNitaplast } from "@/data/nitaplast-irpj-csll-julho";
...
export function DreJulhoCompleta() {
  ...
  const razaoJunho = useMemo(() => junhoControle.aplicar(lancamentosIntegrados), [junhoControle.aplicar]);
  ...
  const irpjCsll = useMemo(() => calcularApuracaoIrpjCsllJulho(dre, irrfAplicacoesFinanceiras), [dre, irrfAplicacoesFinanceiras]);
  ...
  function exportarRazaoCompleto(competencia: "06/2026" | "07/2026") {
    const lancamentosRazao = competencia === "06/2026" ? razaoJunho : razaoAjustado;
    const sufixo = competencia.replace("/", "");
    if (!lancamentosRazao.length) throw new Error(`Nenhum lançamento do Razão foi localizado para ${competencia}.`);

    exportarExcel({
      arquivo: `Lancamentos_Completos_Razao_Nitaplast_${sufixo}.xlsx`,
      aba: "Razão completo",
      titulo: `LANÇAMENTOS COMPLETOS QUE COMPÕEM O RAZÃO — NITAPLAST ${competencia}`,
      subtitulo: `${lancamentosRazao.length} partidas · débitos e créditos provenientes da mesma base do Razão, Balancete e DRE`,
      colunas: [
      ...
      linhas: lancamentosRazao.map((lancamento) => [
  ...
  return <div className="grid gap-5">
    ...
    <Card className="border-amber-500/40 bg-amber-50/40"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">Apuração IRPJ/CSLL — Lucro Real por estimativa mensal — 07/2026</CardTitle><CardDescription>Base = receita bruta × percentual de presunção (indústria/comércio) + demais receitas e ganhos tributados integralmente. Sujeita a ajuste anual pelo LALUR.</CardDescription></div><Badge variant="outline" className="border-amber-600 text-amber-800">SUGERIDO PELO SISTEMA — validar com o contador antes de gerar DARF</Badge></div></CardHeader><CardContent><div className="overflow-x-auto"><table className="w-full text-sm"><tbody>
      <tr className="border-b"><td className="py-2">Receita Bruta da Atividade (indústria/comércio)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.receitaBrutaAtividade)}</td></tr>
      <tr className="border-b"><td className="py-2">(+) Receitas Financeiras (tributadas a 100%)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.receitasFinanceiras)}</td></tr>
      <tr className="border-b"><td className="py-2">(+) Ganho de Capital na Alienação de Imobilizado (tributado a 100%)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.resultadoAlienacaoImobilizado)}</td></tr>
      <tr className="border-b font-semibold"><td className="py-2">Base IRPJ ({(percentualPresuncaoIrpjNitaplast * 100).toFixed(0)}% da receita + outras receitas/ganhos)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.baseIrpj)}</td></tr>
      <tr className="border-b"><td className="py-2">IRPJ normal (15%)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.irpjNormal)}</td></tr>
      <tr className="border-b"><td className="py-2">Adicional IRPJ (10% sobre o que exceder R$ 20.000,00/mês)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.irpjAdicional)}</td></tr>
      <tr className="border-b"><td className="py-2">(-) IRRF sobre Aplicações Financeiras a compensar</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.retencoesIrpjCompensaveis)}</td></tr>
      <tr className="border-b font-bold"><td className="py-2">IRPJ a pagar</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.irpjAPagar)}</td></tr>
      <tr className="border-b font-semibold"><td className="py-2">Base CSLL ({(percentualPresuncaoCsllNitaplast * 100).toFixed(0)}% da receita + outras receitas/ganhos)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.baseCsll)}</td></tr>
      <tr className="border-b"><td className="py-2">CSLL devida (9%)</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.csllDevida)}</td></tr>
      <tr className="font-bold"><td className="py-2">CSLL a pagar</td><td className="py-2 text-right tabular-nums">{brl.format(irpjCsll.csllAPagar)}</td></tr>
    </tbody></table></div><p className="mt-3 text-xs text-muted-foreground">Percentuais de presunção padrão da Lei 9.249/1995 (indústria e comércio, sem revenda de combustíveis ou prestação de serviços). Adições/exclusões específicas do LALUR (ex.: multas indedutíveis, compensação de prejuízo fiscal) ainda não são tratadas neste módulo.</p></CardContent></Card>
