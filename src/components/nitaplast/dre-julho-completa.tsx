...
export function DreJulhoCompleta() {
  ...
  const linhas: Linha[] = [
    ...
    { id: "ajuste-cc503", descricao: "(-) Ajuste de conciliação — CC 503 em Produção/Industrialização", nivel: 1, valor: dre.ajusteConciliacaoCliente.cc503.total, criterio: `${dre.ajusteConciliacaoCliente.cc503.criterio} Não é lançamento do Razão: ajusta só esta linha de conciliação, sem afetar o Balancete. R$ ${Math.abs(dre.ajusteConciliacaoCliente.cc503.producao).toFixed(2)} em Despesas de Produção + R$ ${Math.abs(dre.ajusteConciliacaoCliente.cc503.industrializacao).toFixed(2)} em Despesas com Industrialização.` },
    { id: "ajuste-icms-cofins", descricao: "(+) Ajuste de conciliação — ICMS/COFINS Matriz pendente de documentação", nivel: 1, valor: dre.ajusteConciliacaoCliente.icmsCofinsMatriz.total, criterio: `${dre.ajusteConciliacaoCliente.icmsCofinsMatriz.criterio} Não é lançamento do Razão: ajusta só esta linha de conciliação, sem afetar o Balancete.` },
    { id: "resultado-conciliado", descricao: "(=) Resultado comparável à DRE do cliente", nivel: 0, valor: dre.resultadoConciliadoClienteJulho, criterio: `Resultado do Razão (R$ ${dre.resultado.toFixed(2)}) mais os dois ajustes de conciliação acima (R$ ${dre.ajusteConciliacaoCliente.total.toFixed(2)}). Reproduz o R$ ${resultadoApresentacao.toFixed(2)} da planilha do cliente enquanto os dois pontos ficam pendentes de confirmação documental — nenhum dos dois altera o Razão nem o Balancete.` },
  ];
