...
export function DreJulhoCompleta() {
  ...
  const linhas: Linha[] = [
    ...
    { id: "resultado", descricao: "(=) Resultado do Razão — antes do lançamento de versão", nivel: 0, valor: dre.resultado, criterio: "Resultado do Razão incluindo Mini + Corolla + Transformador. Recebimento do Transformador em parcelas previstas para agosto e setembro/2026, mantido em Duplicatas a Receber. Ainda não reflete o lançamento de versão de agosto/2026 (decisão do contador em 24/08/2026: julho fecha em R$ 234.732,08; a diferença é absorvida por lançamento de versão no mês seguinte, sem reabrir julho)." },
    { id: "ajuste-cc503", descricao: "(-) Lançamento de versão — CC 503 em Produção/Industrialização", nivel: 1, valor: dre.ajusteConciliacaoCliente.cc503.total, criterio: `${dre.ajusteConciliacaoCliente.cc503.criterio} Absorvido por lançamento de versão em agosto/2026, não reabre o Razão de julho. R$ ${Math.abs(dre.ajusteConciliacaoCliente.cc503.producao).toFixed(2)} em Despesas de Produção + R$ ${Math.abs(dre.ajusteConciliacaoCliente.cc503.industrializacao).toFixed(2)} em Despesas com Industrialização.` },
    { id: "ajuste-icms-cofins", descricao: "(+) Lançamento de versão — ICMS/COFINS Matriz", nivel: 1, valor: dre.ajusteConciliacaoCliente.icmsCofinsMatriz.total, criterio: `${dre.ajusteConciliacaoCliente.icmsCofinsMatriz.criterio} Absorvido por lançamento de versão em agosto/2026, não reabre o Razão de julho.` },
    { id: "resultado-conciliado", descricao: "(=) RESULTADO CONTÁBIL 07/2026 — FECHADO", nivel: 0, valor: dre.resultadoConciliadoClienteJulho, criterio: `Resultado do Razão (R$ ${dre.resultado.toFixed(2)}) mais os lançamentos de versão acima (R$ ${dre.ajusteConciliacaoCliente.total.toFixed(2)}). Fechamento de julho/2026 confirmado pelo contador em R$ ${resultadoApresentacao.toFixed(2)}, igual à DRE enviada ao cliente. Os dois pontos que compõem o lançamento de versão são absorvidos em agosto/2026; o Razão de julho não é reaberto.` },
  ];
