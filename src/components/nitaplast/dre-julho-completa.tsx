...
import { lancamentosIntegradosJulhoFinal, resumoFechamentoJulhoFinal } from "@/data/nitaplast-razao-julho-final-v2";
...
export function DreJulhoCompleta(){
  ...
  function mov(c:string){return arred(razaoAjustado.reduce((s,x)=>s+(x.debitoCodigo===c?x.valor:0)-(x.creditoCodigo===c?x.valor:0),0));}
  ...
  const lucroBruto=arred(dre.receitaLiquida-dre.custosReconhecidos);
  const despOper=soma(grupos.despesas);
  ...
  const linhas:Linha[]=[
    ...
    {id:"custos",descricao:"(-) Custos / CPV / CMV",nivel:0,valor:dre.custosReconhecidos,criterio:"Custos resultantes das compras, créditos fiscais e fechamento físico de estoques da matriz e filial.",composicao:grupos.custos},
    {id:"lb",descricao:"(=) LUCRO BRUTO",nivel:0,valor:lucroBruto,criterio:"Receita líquida menos custos contabilizados."},
    {id:"despesas",descricao:"(-) Despesas Operacionais",nivel:0,valor:despOper,criterio:"Despesas operacionais líquidas contabilizadas por documento/centro de custo, incluindo folha, provisões reais e depreciação.",composicao:grupos.despesas},
  ...
  return <div className="grid gap-5">
    <div className="flex flex-wrap items-start justify-between gap-3 border-b pb-4"><div><h1 className="text-xl font-semibold tracking-tight">DRE calculada - Nitaplast 07/2026</h1><p className="mt-1 text-sm text-muted-foreground">A DRE nasce do Razão/Balancete. Lançamentos contábeis e reclassificações manuais de julho recalculam esta demonstração automaticamente.</p></div><div className="flex flex-wrap gap-2"><Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">Consolidado Matriz + Filial</Badge>{ajustesManuais>0?<Badge variant="outline">{ajustesManuais} ajuste(s) manual(is)</Badge>:null}{reclassificacoes.length>0?<Badge variant="outline">{reclassificacoes.length} reclassificação(ões)</Badge>:null}<Button variant="outline" size="sm" onClick={alternarTudo}>{tudo?"Recolher toda DRE":"Expandir toda DRE"}</Button></div></div>
