...
type LinhaReport = {
  ...
  valor: number;
  nivel: 0 | 1;
  tipo: "grupo" | "detalhe" | "subtotal" | "resultado";
};
...
export function DreJulhoReport() {
  ...
  function mov(conta: string) {
    return arred(razaoAjustado.reduce(
      (total, linha) => total + (linha.debitoCodigo === conta ? linha.valor : 0) - (linha.creditoCodigo === conta ? linha.valor : 0),
      0,
    ));
  }
  ...
  const resultadoFinal = arred(resultadoOperacional - despesasFinanceiras + dre.receitasFinanceiras);
  ...
  function exportarCsv() {
    ...
    link.href = url;
    link.download = "Nitaplast_DRE_Report_072026.csv";
    ...
    <>
      <div className="print:hidden">
        <PageHeader
          ...
          descricao="Competência 07/2026 · Razão → Balancete → DRE"
          acoes={
      ...
      <section className="mx-auto w-full max-w-5xl bg-background print:max-w-none print:bg-white print:text-black">
        <header className="border-b-2 border-foreground pb-4 text-center print:hidden">
          ...
          <p className="mt-1 text-sm">Período: 01/07/2026 a 31/07/2026</p>
        </header>
