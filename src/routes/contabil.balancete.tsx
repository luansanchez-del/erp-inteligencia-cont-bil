...
function BalancetePage() {
  ...
  const linhasCalculadas = useMemo<LinhaCalculada[]>(() => {
    ...
    for (const linha of analiticas) {
      ...
      valoresAnaliticos.set(linha.conta, { saldoAnterior, debitos: movimentoConta.debitos, creditos: movimentoConta.creditos, movimento, saldoAtual: saldoAnterior + movimento, lancamentos: movimentoConta.lancamentos });
    }

    return estruturaBalanceteNitaplast.map((linha) => {
      if (linha.tipo === "A") return { ...linha, ...(valoresAnaliticos.get(linha.conta) ?? { saldoAnterior: 0, debitos: 0, creditos: 0, movimento: 0, saldoAtual: 0, lancamentos: 0 }) };
  ...
  const linhas = useMemo(() => {
    ...
    return linhasCalculadas.filter((linha) => {
      ...
      if (!q) return true;
      const corresponde = [linha.conta, linha.classificacao, linha.descricao, linha.tipo].join(" ").toLocaleLowerCase("pt-BR").includes(q);
      return corresponde || (linha.tipo === "S" && correspondenciasAnaliticas.some((analitica) => descendente(analitica, linha)));
    });
  }, [busca, grupo, linhasCalculadas, somenteMovimento]);
  ...
  return <PageShell>
    <PageHeader titulo="Balancete completo - Nitaplast" descricao="Estrutura sintética e analítica do plano contábil, com saldo de implantação em 31/05 e movimento integrado de junho." acoes={<Badge variant="outline">Matriz - 06/2026</Badge>} />
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="Ativo em 31/05" value={resumoImplantacao.ativo} /><Metric label="Passivo + PL em 31/05" value={resumoImplantacao.passivoPatrimonioLiquido} /><Metric label="Linhas do balancete" value={estruturaBalanceteNitaplast.length} /><Metric label="Débitos integrados 06" value={totalDebitosIntegrados} /><Metric label="Créditos integrados 06" value={totalCreditosIntegrados} /></div>
    <Card className="border-emerald-500/40 bg-emerald-500/5"><CardContent className="flex items-start gap-3 pt-6"><CheckCircle2 className="size-5 shrink-0 text-emerald-700" /><div><p className="font-medium">Balancete hierárquico implantado</p><p className="text-sm text-muted-foreground">As linhas S acumulam as contas A abaixo delas. Débitos {brl.format(totalDebitosIntegrados)} e créditos {brl.format(totalCreditosIntegrados)} permanecem equilibrados.</p></div></CardContent></Card>
    <Card>
      <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">Balancete - Débito, Crédito e Movimento</CardTitle><CardDescription>Conta contábil, S/A, classificação e descrição conforme o balancete de origem. Abra o Razão somente nas contas analíticas.</CardDescription></div><div className="relative w-full sm:w-96"><Search className="absolute left-3 top-2.5 size-4 text-muted-foreground" /><Input value={busca} onChange={(event) => { setBusca(event.target.value); setPagina(1); }} placeholder="Buscar conta, classificação ou descrição" className="pl-9" /></div></div><div className="flex flex-wrap gap-2 pt-2">{grupos.map((item) => <button key={item} onClick={() => { setGrupo(item); setPagina(1); }} className={`rounded-md border px-3 py-1.5 text-xs ${grupo === item ? "bg-primary text-primary-foreground" : "bg-background"}`}>{item}</button>)}<button onClick={() => { setSomenteMovimento((valor) => !valor); setPagina(1); }} className={`rounded-md border px-3 py-1.5 text-xs ${somenteMovimento ? "bg-amber-100 text-amber-900" : "bg-background"}`}>Somente com movimento</button></div></CardHeader>
      <CardContent className="overflow-x-auto"><p className="mb-3 text-xs text-muted-foreground">{linhas.length} de {estruturaBalanceteNitaplast.length} linhas exibidas ({estruturaBalanceteNitaplast.filter((linha) => linha.tipo === "S").length} sintéticas e {saldosImplantacao.length} analíticas).</p><table className="w-full min-w-[1450px] text-sm"><thead><tr className="border-b bg-muted/40 text-left text-xs"><th className="p-2">Conta contábil</th><th className="p-2 text-center">S/A</th><th className="p-2">Classificação</th><th className="p-2">Descrição</th><th className="p-2 text-right">Saldo anterior</th><th className="p-2 text-right">Débito</th><th className="p-2 text-right">Crédito</th><th className="p-2 text-right">Movimento</th><th className="p-2 text-right">Saldo atual</th><th className="p-2 text-right">Detalhe</th></tr></thead><tbody>{linhas.slice(inicio, fim).map((linha) => <tr key={`${linha.tipo}-${linha.conta}-${linha.classificacao}`} className={classeLinha(linha)}><td className="p-2 font-mono">{linha.conta}</td><td className="p-2 text-center font-medium">{linha.tipo}</td><td className="p-2 font-mono text-xs">{linha.classificacao}</td><td className="p-2" style={{ paddingLeft: `${8 + Math.max(0, linha.nivel - 1) * 12}px` }}>{linha.descricao}</td><Money value={linha.saldoAnterior} strong={linha.tipo === "S"} /><Money value={linha.debitos} /><Money value={linha.creditos} /><Money value={linha.movimento} /><Money value={linha.saldoAtual} strong /><td className="p-2 text-right">{linha.tipo === "A" ? <Button variant="outline" size="sm" onClick={() => abrirRazao(linha.conta)}>Abrir Razão</Button> : <span className="text-muted-foreground">—</span>}</td></tr>)}</tbody></table></CardContent>
      <CardContent className="flex flex-wrap items-center justify-between gap-3 border-t pt-4"><p className="text-xs text-muted-foreground">Exibindo {linhas.length ? inicio + 1 : 0}-{Math.min(fim, linhas.length)} de {linhas.length} linhas.</p><div className="flex items-center gap-2"><Button variant="outline" size="sm" disabled={paginaAtual <= 1} onClick={() => setPagina((valor) => Math.max(1, valor - 1))}>Anterior</Button><span className="text-xs">Página {paginaAtual} de {totalPaginas}</span><Button variant="outline" size="sm" disabled={paginaAtual >= totalPaginas} onClick={() => setPagina((valor) => Math.min(totalPaginas, valor + 1))}>Próxima</Button></div></CardContent>
    </Card>
  </PageShell>;
}
