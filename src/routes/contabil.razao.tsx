...
import { lancamentosIntegrados, lancamentosPorRastreio, totalCreditosIntegrados, totalDebitosIntegrados } from "@/data/nitaplast-razao-integrado";
...
function RazaoPage() {
  ...
  }, [busca, contaSelecionada, origem, rastreio, status]);
  ...
  return <PageShell>
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <Metric label="Sem lastro documental" value={lancamentosPorRastreio.derivado + lancamentosPorRastreio.sugerido} warning />
      <Metric label="Com lastro documental" value={lancamentosPorRastreio.documento} />
    </div>
    <Card className="border-emerald-500/40 bg-emerald-500/5"><CardContent className="pt-5 text-sm"><strong>Rastreabilidade:</strong> todo lançamento carrega a fonte que o comprova. <em>Documento</em> = linha de arquivo recebido; <em>Derivado</em> = calculado a partir de arquivo recebido (rateio, padrão do razão anterior); <em>Sugerido</em> = classificação proposta, sem lastro — não pode ser exportada sem confirmação. Nenhum valor é criado sem origem.<br /><strong>Critério aplicado:</strong> movimento bancário e folha são de junho; códigos e descrições vêm do balancete; razão anterior é referência apenas para depreciação, JCP e abertura de folha por centro de custo.</CardContent></Card>
    <Card>
