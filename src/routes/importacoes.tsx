...
function ImportacoesPage() {
  ...
  async function receber(lista: FileList | null) {
    ...
    const proximos = [...itens]; let adicionados = 0; let duplicados = 0; let lidos = 0;
    ...
    persistir(proximos);
    setMensagem(`${adicionados} arquivo(s) registrado(s)${duplicados ? ` · ${duplicados} duplicado(s) bloqueado(s)` : ""}${lidos ? ` · ${lidos} extrato(s) já lido(s) automaticamente (aprove o arquivo pra ver a prévia)` : ""}. Nenhum lançamento foi criado ainda.`);
  ...
  function marcarLancamentosGerados(id: string, ids: string[]) { persistir(itens.map((item) => (item.id === id ? { ...item, lancamentosGeradosIds: ids } : item))); }
  const itemPrevia = previaAbertaId ? contexto.find((item) => item.id === previaAbertaId) : undefined;
